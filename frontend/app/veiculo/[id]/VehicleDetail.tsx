...
export default function VehicleDetail({ id }: { id: string }) {
  ...
  const whatsappLink = vehicle.dealership.phone
    ? `https://wa.me/55${vehicle.dealership.phone.replace(/\D/g, '')}?text=${encodeURIComponent(
        `Olá! Encontrei no BusCAR o anúncio do ${vehicle.brand} ${vehicle.model}${
          vehicle.version ? ` ${vehicle.version}` : ''
        } ${vehicle.year}, por ${formatPrice(vehicle.price)}. Tenho interesse no carro`
      )}`
    : null;
          ...
          <div className="aspect-[16/10] sm:aspect-auto sm:h-80 bg-stone-100 relative">
            {vehicle.photos.length > 0 ? (
              <>
                ...
                <span className="absolute bottom-3 left-3 text-xs font-medium px-2 py-1 rounded-md bg-stone-900/70 text-white">
                  Clique para ampliar
                </span>
